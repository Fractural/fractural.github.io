...
<template>
  <div class="relative" :style="heroStyle">
    <video ref="heroVideo" :class="{
      hidden: !(props.video && !lowPowerMode),
    }" loop muted autoplay class="absolute w-full h-full object-cover" :src="props.video"></video>
    <img :class="{
      hidden: !(
        props.image &&
        (!props.video || (props.video && lowPowerMode))
      ),
    }" class="absolute w-full h-full object-cover" :src="props.image" />
    <div class="absolute w-full h-full bg-linear-to-t from-black"></div>
