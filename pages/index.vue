...
import heroVideoThumbnail from '~/assets/images/hero_montage_thumbnail.png';
...
const getGamesGridStyle = computed(() => {
  let width = '17em';
  if (aboveBreakpoint('xsm', breakpoints.is)) width = '20em';
  return <any>{
...
<template>
  <SectionHero :video="heroVideoUrl" :image="heroVideoThumbnail">
  ...
  <SectionBody>
    ...
    <div class="grid items-stretch gap-4" :style="getGamesGridStyle">
      ...
      >
        ...
        />
        <div
          class="absolute bg-gradient-to-t from-black from-50% w-full h-full"
        ></div>

        <div
          class="relative w-full p-8 flex flex-col"
          style="height: calc(100% - 20rem); margin-top: 20rem"
        >
          ...
          <div class="flex flex-row flex-wrap gap-2">
            <img
              v-if="game.platforms.includes('googlePlay')"
              src="~/assets/images/googleplay_badge.png"
              class="h-4 xs:h-8 lg:h-10"
            />
            <img
              v-if="game.platforms.includes('appStore')"
              src="~/assets/images/appstore_badge.png"
              class="h-4 xs:h-8 lg:h-10"
            />
            <img
              v-if="game.platforms.includes('steam')"
              src="~/assets/images/steam_badge.png"
              class="h-4 xs:h-8 lg:h-10"
            />
            <img
              v-if="game.platforms.includes('itchio')"
              src="~/assets/images/itchio_badge.png"
              class="h-4 xs:h-8 lg:h-10"
            />
          </div>
        </div>
